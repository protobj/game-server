...
public class IntNodeBinaryQueue<T extends IntNodeBinaryQueue.Node> {
    /**
    ...
    public IntNodeBinaryQueue() {
        this(0, false);
    }

    public IntNodeBinaryQueue(int capacity, boolean isMaxHeap) {
    ...
    private void down(int index) {
        ...
        while (true) {
            ...
            if (rightIndex >= size) {
                rightNode = null;
                rightValue = isMaxHeap ? Integer.MIN_VALUE : Integer.MAX_VALUE;
            } else {
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IntNodeBinaryQueue))
            return false;
        IntNodeBinaryQueue other = (IntNodeBinaryQueue) obj;
        if (other.size != size)
            return false;
        for (int i = 0, n = size; i < n; i++)
