...
public class MockManagerTcpServer {
    ...
    public static class MockManagerData {
        private Map<Integer, MockInstanceData> mockInstanceData = CommonUtil.createSimpleMap();
            ...
            try {
                ...
                if (next.startsWith("start")) {
                    ...
                    if (!reportVos.isEmpty()) {
                        ...
                        try {
                            Files.write(reportVO.toCsv().getBytes(StandardCharsets.UTF_8), new File(property + "\\mock_" + NDateUtil.sdf.get().format(new Date()) + ".csv"));
                        } catch (IOException e) {
