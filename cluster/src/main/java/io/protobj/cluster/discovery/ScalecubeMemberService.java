...
public class ScalecubeMemberService implements MemberService, ClusterMessageHandler {
    private static final Logger logger = LoggerFactory.getLogger(ScalecubeMemberService.class);
    ...
    public ScalecubeMemberService() {
        this.clusterConfig = ClusterConfig.defaultLanConfig();
    }

    private ScalecubeMemberService(ScalecubeMemberService other) {
        this.clusterConfig = other.clusterConfig;
        this.cluster = other.cluster;
    }

    public ScalecubeMemberService options(UnaryOperator<ClusterConfig> opts) {
        ScalecubeMemberService d = new ScalecubeMemberService(this);
        d.clusterConfig = opts.apply(clusterConfig);
        return d;
    }

    public ScalecubeMemberService transport(UnaryOperator<TransportConfig> opts) {
        return options(cfg -> cfg.transport(opts));
    }

    public ScalecubeMemberService membership(UnaryOperator<MembershipConfig> opts) {
        return options(cfg -> cfg.membership(opts));
    }

    public ScalecubeMemberService gossip(UnaryOperator<GossipConfig> opts) {
        return options(cfg -> cfg.gossip(opts));
    }


    public ScalecubeMemberService failureDetector(UnaryOperator<FailureDetectorConfig> opts) {
